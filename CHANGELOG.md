...
- Ampache API: The standard action `folders` of API8, which browses the folder tree of the library one level at a time, addressing the folders either by ID or by path name; the root of the library is the ID -1 or the path `/`
  * Our own flat listing of all the folders carried this name before API8 defined an action of its own, and is now called `folders_flat`. It is a proprietary extension which was never documented, and only the Dashboard widget of this app uses it
